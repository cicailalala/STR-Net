...
def make_testset(root):
    imgs = []
    img_labels = {}
    img_names = os.listdir(root +'tnus/test-image/')
    img_names = sorted(img_names, key=lambda i: int(i.split(".")[0]))

    for img_name in img_names:
        img = os.path.join(root +'tnus/'+ name+ '-image/', img_name)
        mask = os.path.join(root +'tnus/'+ name+ '-mask/', img_name)
        imgs.append((img, mask))
    return imgs


class TATU(data.Dataset):
    def __init__(self, mode, transform=None, return_size=False, fold=0, root='./data/'):
        self.mode = mode
        nodule_trainval = json.load(open(root + '/tnus/part2_for_seg/tnus-trainval-fold' + str(fold) + '.json', 'r'))  # seeds for k-fold cross validation
        ...
        if mode == 'train':
            nodule_pathes = make_dataset(root, nodule_trainval['train'], 'tnus/part2_for_seg/trainval')
            ...
            for i in range(len(gland_pathes)):
                gland_imgs.append(gland_pathes[i])

        elif mode == 'val':
            nodule_imgs = make_dataset(root, nodule_trainval['val'], 'tnus/part2_for_seg/trainval')
    ...
    def __getitem__(self, item):
        if self.mode == 'train':
            ...
            # Gland
            idx = np.random.choice(len(self.gland_imgs))
            image_path, label_path = self.gland_imgs[idx]
            assert os.path.exists(image_path), ('{} does not exist'.format(image_path))
            assert os.path.exists(label_path), ('{} does not exist'.format(label_path))
